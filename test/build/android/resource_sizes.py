...
def PrintPakAnalysis(apk_filename, min_pak_resource_size):
    ...
    print
    print
    'Analyzing pak files in %s...' % apk_filename
    ...
    print
    'Total pak files: %d' % total_files
    print
    'Total compressed size: %s' % _FormatBytes(total_compress_size)
    print
    'Total uncompressed size: %s' % _FormatBytes(total_file_size)
    print
    ...
    print
    print
    'Analyzing pak resources in %s...' % apk_filename
    ...
    print
    'Total pak resources: %s' % total_resource_count
    print
    'Total uncompressed resource size: %s' % _FormatBytes(
        total_resource_size)
    print
    ...
    print
    print
    '%56s %5s %17s' % ('RESOURCE', 'COUNT', 'UNCOMPRESSED SIZE')
...
def _GetResourceIdNameMap():
    ...
    print
    'Looking at resources in: %s' % out_dir
            ...
            for line in f.readlines():
                ...
                if m:
                    ...
                    if i in id_name_map and name != id_name_map[i]:
                        print
                        'WARNING: Resource ID conflict %s (%s vs %s)' % (
...
def _PrintStaticInitializersCountFromApk(apk_filename, chartjson=None):
    print
    'Finding static initializers (can take a minute)'
...
def _PrintStaticInitializersCount(so_with_symbols_path):
    ...
    if si_count != static_initializers_count:
        ...
        print
        '%s - Found %d files with static initializers:' % (
